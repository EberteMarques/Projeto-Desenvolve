import random

lista_original = []

for _ in range(20):
    valor_aleatorio = random.randint(-100, 100)
    lista_original.append(valor_aleatorio)

lista_ordenada = sorted(lista_original)
print("A lista ordenada (sem modificar a original):", lista_ordenada)

print("A lista original:", lista_original)

maior_valor = max(lista_original)
indice_maior = lista_original.index(maior_valor)
print(f"O índice do maior valor ({maior_valor}) na lista original é: {indice_maior}")

menor_valor = min(lista_original)
indice_menor = lista_original.index(menor_valor)
print(f"O índice do menor valor ({menor_valor}) na lista original é: {indice_menor}")